...
const NearByNurseComp = ({ nurseName, nurseRate, nurseLikes, nurseSpecialite, patientClients, profilePic, price, choosenNurseName, setChoosenNurseName}) => {
  ...
  return (
      <div className={`flex items-center gap-3 w-full p-2 rounded-20 shadow-panelShadow ${choosenNurseName === nurseName ? 'bg-blueketba' : 'bg-white' }`} onClick={selectNurse}>
            ...
            <div className="infosRight flex justify-between items-center h-[100px] flex-1 pr-2"> {/*relative*/}
              <div className="rightLeft flex flex-col justify-between h-full">
                <div className="nameSpec">
                   <p className={`${choosenNurseName === nurseName ? 'text-creme2' : 'text-blueketba' } font-[450]`}>{nurseName}</p>
                ...
                <div className="stats flex items-center gap-5">
                  <div className="stat flex flex-col gap-1 items-center">
                      <FontAwesomeIcon icon={faUsers} className={`${choosenNurseName === nurseName ? 'text-creme2' : 'text-blueketba' } text-sm`} />
                      <span className={`${choosenNurseName === nurseName ? 'text-[#a0a1a4]' : 'text-blueketba' } text-sm`}>{patientClients}</span>
                  </div>
                  <div className="stat flex flex-col gap-1 items-center">
                      <FontAwesomeIcon icon={faStar} className={`${choosenNurseName === nurseName ? 'text-creme2' : 'text-blueketba' } text-sm`}/>
                      <span className={`${choosenNurseName === nurseName ? 'text-[#a0a1a4]' : 'text-blueketba' } text-sm`}>{nurseRate}</span>
                  </div>
                  <div className="stat flex flex-col gap-1 items-center">
                      <FontAwesomeIcon icon={faHeart} className={`${choosenNurseName === nurseName ? 'text-creme2' : 'text-blueketba' } text-sm`}/>
                      <span className={`${choosenNurseName === nurseName ? 'text-[#a0a1a4]' : 'text-blueketba' } text-sm`}>{nurseLikes}</span>
                  </div>
                </div>
              </div>

               <div className={`prix ${choosenNurseName === nurseName ? 'text-creme2' : 'text-blueketba' } font-[500]`}> {/*absolute right-0*/}
                    {price} DZD  
               </div>
            </div> 
      </div>
